...
# 35 "room/adv_guild.c" 
reset(arg) { extra_reset(arg); if (arg) return; set_light( 1); }short() { if (set_light(0)) return  	 "The adventurers guild"; return "dark room";}init() { add_action("move",  "north"); add_action("cost_for_level", "cost"); add_action("advance", "advance"); add_action("south", "south"); add_action("banish", "banish"); add_action("list_quests", "list");}move() {  call_other(this_player(), "move_player",  "north" + "#" +"room/vill_road2"); return 1;}long(str) { if (set_light(0) == 0){ write("It is dark.\n"); return; }  write( 	 "You have to come here when you want to advance your level.\n" + 	 "You can also buy points for a new level.\n" + 	 "Commands: cost, advance [level, str, dex, int, con], list (number).\n" + 	 "There is an opening to the south, and some shimmering\n" + 	 "blue light in the doorway.\n"); write("    The only obvious exit is " +  "north" + ".\n");}
...
cost_for_level()
{
    ...
    write("It will cost you "); write((next_exp - exp) * 1000 / 600);
