...
shout_to_all(str) {
    ...
    if (!frog) { gTellstring=cap_name + " shouts: " + str + "\n"; filter_objects(users(),"filter_tell",this_object()); }
    else {
      gTellstring=cap_name + " the frog shouts: " + "Hriibit! Hrriiibit!" + "\n"; filter_objects(users(),"filter_tell",this_object());
    }
    write("Ok.\n");
    return 1;
}
...
add_exp(e) {
    ...
    if (this_player() && this_player() != this_object() &&
      query_ip_number(this_player()) && level < 20 && e >= 10000)
...
more(str) {
    ...
    more_file = str;
    more_line = 1;
    if (cat(more_file, more_line, 16) == 0) {
	write("No such file\n");
	return 1;
    }
    input_to("even_more");
    write("More: (line " + (16 + 1) + ") ");
    return 1;
}
