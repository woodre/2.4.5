...
history_add(str) {
    ...
    if(counter >= 40 || verb == "get" || verb == "take") {
	counter = 0;
	if(!needs_refresh) {
	    needs_refresh = 1;
	    if(is_debug) { 
...
version(str) {
    ...
    write("Tech's quicktyper version " + "2.06" + " created " + "901020" + "\n");
    return 1;
}
